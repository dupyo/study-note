...
2. ### CPU의 조직과 CPU 논리회로
   ...
   **제어장치**
   ...
   <figure style="text-align: center;"><img src="C:\Users\user\AppData\Roaming\Typora\typora-user-images\image-20211227161810418.png" alt="제어장치의 구성" style="zoom:50%;" /><figcaption>제어장치의 구성</figcaption></figure>
...
3. ### CPU의 기능과 동작 그리고 성능
   ...
   - 주기억장치에서 버스를 통해 명령어를 기억장치 버퍼 레지스터로 전달
   ...
   - 최종적으로 연산 결과를 주기억장치로 보내 저장

   

   CPU가 데이터를 처리하는 속도를 판단하는 요소

   \- 클럭 주파수, 워드 크기, 캐시 메모리, 명령어 집합의 복잡성, 파이프라이닝, 병렬처리

   - 클럭 주파수 - 클럭 주기가 짧을수록 클럭 주파수가 높음. 

     특정 시간동안 완수할 수 있는 명령어의 수가 더 정확함

   - 워드 크기 - CPU가 한번에 읽고 쓸 수 있는 비트 수. 레지스터의 크기와 버스 데이터 선로 수

   - 캐시 메모리 - 읽기 쓰기 속도 향상. CPU가 빠르게 접근 가능한 고속 기억장치(빈번한 명령어 보관)

   - 명령어 집합의 복잡성(RISC) - 연산속도를 향상시키기 위한 단순화된 명령어 구조

   - 파이프라이닝 - CPU 또는 프로세서가 이전 명령어 수행이 종료되기 전에 다음 명령어 수행을 시작. CPU 처리 속도 증가
